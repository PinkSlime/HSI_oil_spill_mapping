...
import time
import os
def  wzh_oil(input,TE,TR,GT,my_colormap) :
    parser = argparse.ArgumentParser("HSI")
    parser.add_argument('--dataset', choices=['Indian', 'Pavia', 'Houston','wzh'], default='wzh', help='dataset to use')
    ...
    parser.add_argument('--patches', type=int, default=2    , help='number of patches')
    ...
    elif args.dataset == 'wzh':
        color_mat=my_colormap
        TR=TR
        TE=TE
        input=input
        label=GT
    ...
    out_put = []
    out_put = np.append(out_put, AA_mean2)
    out_put = np.append(out_put, OA2)
    out_put = np.append(out_put, Kappa2)
    out_put = np.append(out_put, AA2[:])
    out_put = np.append(out_put,toc-tic)

    prediction_matrix[GT==0]=0

    return out_put, prediction_matrix
